...
function CourseActions({ course, compact = false }: { course: Course; compact?: boolean }) {
    const secondary = "inline-flex h-9 items-center justify-center rounded-xl border border-[#3A3A3A]/10 text-[#3A3A3A]/60 transition hover:border-[#F47822]/35 hover:bg-[#F47822]/7 hover:text-[#F47822] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#F47822]/35";
    return <div className={`mt-5 flex items-center gap-2 ${compact ? "mt-0 flex-wrap lg:justify-end" : ""}`}><Link to={`/instructor/courses/${course.id}`} className="inline-flex h-9 flex-1 items-center justify-center gap-1.5 rounded-xl bg-[#F7F7F7] px-3.5 text-xs font-bold text-white shadow-[0_5px_12px_rgba(58,58,58,.12)] transition hover:bg-[#F47822] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#F47822]/40"><FilePenLine className="h-3.5 w-3.5" />Manage<span className="hidden sm:inline"> course</span></Link><Link title="Course analytics" aria-label={`View ${course.title} analytics`} to={`/instructor/courses/${course.id}/analytics`} className={`${secondary} w-9`}><BarChart3 className="h-3.5 w-3.5" /></Link><Link title="Learner outcomes" aria-label={`View ${course.title} outcomes`} to={`/instructor/courses/${course.id}/outcomes`} className={`${secondary} w-9`}><MessageSquareText className="h-3.5 w-3.5" /></Link>{course.status === "published" && <Link title="Preview course" aria-label={`Preview ${course.title}`} to={`/courses/${course.id}`} className={`${secondary} w-9`}><ExternalLink className="h-3.5 w-3.5" /></Link>}</div>;
}
