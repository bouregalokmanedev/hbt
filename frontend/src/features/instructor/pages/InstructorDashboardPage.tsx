...
export function InstructorDashboardPage() {
    ...
    if (isError || !data) {
        return <div className="rounded-2xl border border-red-200 bg-red-50 p-6"><h2 className="text-lg font-semibold text-red-900">Unable to load instructor dashboard</h2><p className="mt-2 text-sm text-red-700">We couldn't retrieve your teaching insights.</p><button type="button" onClick={() => refetch()} className="mt-5 inline-flex h-10 items-center gap-2 rounded-xl bg-[#F47822] px-4 text-xs font-bold text-white shadow-[0_8px_18px_rgba(244,120,34,.22)] transition-all duration-200 hover:-translate-y-0.5 hover:bg-[#de6414] hover:shadow-[0_12px_25px_rgba(244,120,34,.28)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#F47822]/40"><RefreshCw className="h-4 w-4" />Try again</button></div>;
    }
    ...
    const checklist = [
        { complete: data.statistics.published > 0, text: "Publish a course learners can enroll in" },
        { complete: data.statistics.draft === 0 || data.statistics.total === 0, text: "Review unfinished course drafts" },
        { complete: data.students.total > 0, text: "Welcome your first enrolled learner" },
        { complete: data.learning.average_quiz_score > 0, text: "Add a checkpoint quiz for learners" },
        { complete: data.overview.average_progress > 0, text: "Track your first learner milestone" },
        { complete: data.students.active > 0, text: "Keep an active learner engaged" },
        { complete: data.overview.completion_rate >= 50, text: "Reach a 50% course completion rate" },
    ];
        ...
        <div className="mx-auto max-w-7xl space-y-6 lg:space-y-7">
            <section className="overflow-hidden rounded-3xl bg-[#3A3A3A] px-6 py-7 text-white shadow-[0_18px_45px_rgba(58,58,58,.14)] sm:px-8 sm:py-8">
                <div className="flex flex-col justify-between gap-8 xl:flex-row xl:items-end">
                    ...
                    <div className="flex flex-wrap items-center gap-3"><Link to="/instructor/courses/new" className="inline-flex h-11 items-center gap-2 rounded-xl bg-[#F47822] px-4 text-xs font-bold text-white shadow-[0_8px_20px_rgba(244,120,34,.25)] transition-all duration-200 hover:-translate-y-0.5 hover:bg-[#de6414] hover:shadow-[0_13px_28px_rgba(244,120,34,.32)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50"><FilePlus2 className="h-4 w-4" />Create course</Link><Link to="/instructor/students" className="inline-flex h-11 items-center gap-2 rounded-xl border border-white/20 bg-white/[.06] px-4 text-xs font-bold text-white transition-all duration-200 hover:-translate-y-0.5 hover:border-white/35 hover:bg-white/[.14] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50"><Users className="h-4 w-4" />View learners</Link></div>
                </div>
            ...
            <div className="grid gap-6 xl:grid-cols-[1.15fr_.85fr]">
                ...
                <section className="rounded-2xl border border-[#F47822]/20 bg-[#FFF8F4] p-5 sm:p-6"><SectionHeading icon={BarChart3} title="Learning pulse" subtitle="How learners are moving through your teaching." /><div className="mt-6 grid grid-cols-2 gap-3"><PulseCard label="In progress" value={data.progress.in_progress} icon={Clock3} /><PulseCard label="Completed" value={data.progress.completed} icon={CheckCircle2} /><PulseCard label="New this month" value={data.students.new_this_month} icon={UserPlus} /><PulseCard label="Avg. progress" value={`${data.overview.average_progress}%`} icon={BarChart3} /></div><Link to="/instructor/students" className="mt-5 flex h-11 items-center justify-between rounded-xl border border-[#F47822]/15 bg-white px-4 text-xs font-bold text-[#3A3A3A] shadow-[0_5px_14px_rgba(58,58,58,.05)] transition-all duration-200 hover:-translate-y-0.5 hover:border-[#F47822]/35 hover:bg-[#F47822] hover:text-white hover:shadow-[0_10px_22px_rgba(244,120,34,.2)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#F47822]/35"><span>Review learner progress</span><ArrowRight className="h-4 w-4" /></Link></section>
            </div>
            ...
            <div className="grid gap-6 xl:grid-cols-[1.2fr_.8fr]">
                ...
                <section className="relative overflow-hidden rounded-2xl border border-[#3A3A3A]/8 bg-white p-5 shadow-[0_10px_30px_rgba(58,58,58,.045)] sm:p-6"><div className="relative flex items-start justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-[#F47822]">Teaching checklist</p><h2 className="mt-2 text-lg font-semibold tracking-tight text-[#3A3A3A]">Keep your library moving</h2><p className="mt-2 text-sm leading-6 text-[#3A3A3A]/50">A few focused steps to turn your content into a stronger learning journey.</p></div><div className="shrink-0 rounded-xl border border-[#F47822]/15 bg-[#FFF8F4] px-3 py-2 text-center"><p className="text-lg font-bold leading-none text-[#F47822]">{checklistDone}/{checklist.length}</p><p className="mt-1 text-[9px] font-bold uppercase tracking-[.1em] text-[#3A3A3A]/45">Complete</p></div></div><div className="relative mt-5 h-1.5 overflow-hidden rounded-full bg-[#3A3A3A]/8"><div className="h-full rounded-full bg-[#F47822] transition-[width] duration-500" style={{ width: `${(checklistDone / checklist.length) * 100}%` }} /></div><div className="relative mt-4 space-y-2">{checklist.map((item) => <ChecklistItem key={item.text} complete={item.complete} text={item.text} />)}</div><Link to="/instructor/courses" className="relative mt-6 flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#F47822] px-4 text-xs font-bold text-white shadow-[0_8px_18px_rgba(244,120,34,.2)] transition-all duration-200 hover:-translate-y-0.5 hover:bg-[#de6414] hover:shadow-[0_12px_26px_rgba(244,120,34,.28)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#F47822]/35">Open course library <ArrowRight className="h-3.5 w-3.5" /></Link></section>
            </div>
        </div>
    );
}
