...
import {
    LessonVideoPlayer,
} from "../components/LessonVideoPlayer";
...
function LessonPlayerContent({
    ...
    const navigationLessons = curriculum?.sections.flatMap(
        (section) => section.lessons,
    ) ?? [];
    ...
    const isCompleted = lesson.progress?.is_completed === true || lesson.progress?.completed_at != null;

    return (
        <div className="min-h-screen bg-[#F7F7F7]">
            <LessonMentorPopup lessonTitle={lesson.title} lessonId={lesson.id} courseId={courseId} />
            {/* =====================================================
                LEARNING HEADER
            ====================================================== */}
                    ...
                    <section className="min-w-0">
                        <div className="mb-4 flex flex-col gap-3 rounded-2xl border border-gray-200 bg-white px-4 py-3 shadow-[0_5px_20px_rgba(15,23,42,0.035)] sm:flex-row sm:items-center sm:justify-between sm:px-5">
                            <div className="flex min-w-0 items-center gap-3">
                                <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${isCompleted ? "bg-emerald-50 text-emerald-600" : "bg-[#F47822]/10 text-[#F47822]"}`}>
                                    {isCompleted ? <CheckCircle2 className="h-5 w-5" /> : <PlayCircle className="h-5 w-5" />}
                                </div>
                                <div className="min-w-0">
                                    <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#F47822]">Your learning session</p>
                                    <p className="truncate text-sm font-semibold text-[#3A3A3A]">{isCompleted ? "Lesson completed — review anytime" : progressPercentage > 0 ? "In progress — pick up where you left off" : "Ready when you are"}</p>
                                </div>
                            </div>
                            <div className="flex shrink-0 items-center gap-3 text-xs text-gray-500 sm:border-l sm:border-gray-100 sm:pl-4">
                                <span className="inline-flex items-center gap-1.5"><Clock3 className="h-4 w-4 text-[#F47822]" />{lesson.duration_minutes} min</span>
                                <span className={`rounded-full px-2.5 py-1 font-bold ${isCompleted ? "bg-emerald-50 text-emerald-700" : "bg-[#F47822]/10 text-[#F47822]"}`}>{isCompleted ? "Completed" : `${progressPercentage}% complete`}</span>
                            </div>
                        </div>
                        ...
                        {/* Lesson information */}
                        <article className="mt-6 overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-[0_8px_28px_rgba(15,23,42,0.045)]">
