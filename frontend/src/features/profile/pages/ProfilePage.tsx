...
export function ProfilePage() {
    ...
    const [profileBadges, setProfileBadges] = useState<Array<{ id: string; title: string; icon: string }>>([]);
    ...
    */
    ...
    useEffect(() => {
        void dashboardApi.getDashboard()
            .then((data) => {
                setDashboard(data);
                setProfileBadges(data.achievements.filter((achievement) => achievement.completed).slice(-3).reverse());
            })
            .catch(() => {
                setDashboard(null);
                setProfileBadges([]);
